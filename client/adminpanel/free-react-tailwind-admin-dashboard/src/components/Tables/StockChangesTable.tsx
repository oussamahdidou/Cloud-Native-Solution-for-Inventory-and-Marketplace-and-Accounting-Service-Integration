...
import { StockIO } from '../../types/types';

interface Props {
  ioStock: StockIO[];
}

const StockChangesTable = (props: Props) => {
  return (
        ...
        <tbody>
          {props.ioStock.map((item) => (
            <tr className="odd:bg-white odd:dark:bg-gray-900 even:bg-gray-50 even:dark:bg-gray-800 border-b dark:border-gray-700">
              ...
              >
                <span
                  className={` flex justify-center items-center py-2 px-3 rounded-3xl text-white ${
                    item.type === 'Sortie' ? 'bg-red-500' : 'bg-green-500'
                  }`}
                >
                  {item.type === 'Sortie' ? (
                    <svg
              ...
              <td className="px-6 py-4">{new Date(item.Date).toString()}</td>
...
export default StockChangesTable;
